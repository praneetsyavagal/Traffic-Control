...
def do_action(n,phno):
    ph = (phno + 1) % 4
    #arr_act = [[5,5],[5,10],[5,15],[10,5],[10,10],[10,15],[15,5],[15,10],[15,15],]
    dur=(n+1)*5
    #traci.trafficlight.setPhase("0", ph)
    step = 0
    while step < dur:
        traci.trafficlight.setPhase("n2", ph)
        traci.trafficlight.setPhase("n3", ph)
...
def generate_routefile():
    ...
    pEW = 1. / 11
    pNS = 1. / 30
    with open("new/my_route.rou.xml", "w") as routes:
        print("""<routes>
        <vType id="typeWE" accel="0.8" decel="4.5" sigma="0.5" length="5" minGap="2.5" maxSpeed="16.67" guiShape="passenger"/>
        <vType id="typeNS" accel="0.8" decel="4.5" sigma="0.5" length="7" minGap="3" maxSpeed="25" guiShape="bus"/>

        <route id="route0" edges="1to2 2to0 0to3 3to8 "/>
        <route id="route1" edges="8to3 3to0 0to2 2to1 "/> 
        <route id="route2" edges="4to2 2to6 "/> 
        <route id="route3" edges="6to2 2to4"/>  
        <route id="route4" edges="5to3 3to7"/>
        <route id="route5" edges="7to3 3to5"/>""", file=routes)
        lastVeh = 0
        vehNr = 0
        for i in range(N):
...
#   ======================================================================================================================== 
    ...
    traci.start([sumoBinary1, "-c", "new/my_config_file.sumocfg",
    ...
    with tf.Session() as sess:
        ...
        for i in range(num_episodes):
            ...
            while j < 3333:
                ...
                #x[s] = 1
                #print('x = ',x[s])
                a,allQ = sess.run([predict,Qout],feed_dict={inputs1:s_vec.reshape((1,state_space_size))})
                ...
                #x1[s1] = 1
                Q1 = sess.run(Qout,feed_dict={inputs1:s1_vec.reshape((1,state_space_size))})
                #Obtain maxQ' and set our target value for chosen action.
                maxQ1 = np.max(Q1)
                targetQ = allQ
                targetQ[0,a[0]] = r + y*maxQ1
                #Train our network using target and predicted Q values
                _,w,w1,w2,los = sess.run([updateModel,W,W1,W2,loss],feed_dict={inputs1:s_vec.reshape((1,state_space_size)),nextQ:targetQ})
                some = np.array([2,2,2,2,2,2,2,2,2,2,2,2])
    ...
    #print("Percent of succesful episodes: " + str(sum(rList)/num_episodes) + "%")
        ...
        w = sess.run(W)
        print("after w = ", w)
        some = np.array([123,434,54,54,44,44,54,34,12,32,43,65])
        a1 = sess.run(predict,feed_dict={inputs1:some.reshape((1,state_space_size))})
        '''print("\n\n\n\n\n")
        print("before a - ", a1)
        print("\n\n\n\n\n")'''
    ####################################################################$$############################################################
    ...
    #with tf.Session() as sess:
        ...
        while step < 1000:
            ...
            #x = np.zeros(state_space_size)
            #x[s] = 1
            a = sess.run(predict,feed_dict={inputs1:s_vec.reshape((1,state_space_size))})
    ...
    ##################################################################$$############################################################
        ...
        for du in [5,10,15]:
            ...
            with open("result_new_1.txt", "a") as myfile:
